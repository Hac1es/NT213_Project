<template>
  <div>
    <h2 class="text-[#63e2b7] text-3xl font-black mb-8">
    ...
    <v-chart
      class="w-full"
      style="height: 400px"
      :option="getChartOption(impactScopeData)"
      autoresize
    />
    ...
    <div class="overflow-x-auto p-6 rounded-2xl mt-5">
      <table class="w-full text-left border-collapse">
        <thead>
          ...
          >
            <th class="pb-4 font-black">Impact Scope</th>
        ...
        <tbody class="divide-y divide-gray-800/30">
          <tr
            v-for="(scope, idx) in rankedImpactScopes"
            :key="scope.name"
            class="hover:bg-white/5"
          >
            <td class="py-4">
              <span class="text-gray-200 font-bold">{{ scope.name }}</span>
            </td>
...
import impactScopeMapping from "../assets/impact_scope_mapping.json";
...
const impactScopeData = computed(() => {
  return Object.entries(impactScopeMapping).map(([scopeName, requirements]) => {
...
const rankedImpactScopes = computed(() => {
  return [...impactScopeData.value].sort(
